/// '/set <role>' command implementation. 
...
pub async fn set_reg_role(
	ctx: Context<'_>,
	#[description = "The role to assign to users when they register"] role: serenity::Role,
) -> Result<(), Error> {
	debug!("Invoked /set_reg_role with role {} (ID {})", role.name, role.id);
	let guild_id = ctx
		.guild_id()
	...
	// Update the registered role ID in the config and save it to the database.
	guild_config.registered_role_id = Some(role.id.get());
	info!("Setting registration role for guild {} to {} (ID {})", guild_id, role.name, role.id);
	...
	debug!("Finished handling /set_reg_role");
